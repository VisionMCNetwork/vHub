...
public class PracticeServerSelectorMenu extends Menu {
    @Override
    public String getTitle(Player player) {
        return MessageUtil.translate(Hub.getInstance().getServerSelectorConfig().getString("server-selector.practice-inventory.title"));
    }

    @Override
    public int getSize() {
        int rows = Hub.getInstance().getServerSelectorConfig().getInt("server-selector.practice-inventory.rows");
        return 9 * rows;
    }
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        ConfigurationSection itemSection = serverSelectorConfig.getConfigurationSection("server-selector.practice-inventory.items");
        ...
        for (String itemKey : itemSection.getKeys(false)) {
            ...
            buttons.put(currentItemSection.getInt("slot"), new PracticeServerButton(currentItemSection));
        }
